...
function BillsList() {
  ...
  const getStatusClass = (status) => {
    return status === "PAID" ? "text-green-700" : "text-red-600";
  };
          ...
          {payments.length > 0 ? (
            payments.map((payment) => (
              <tr key={payment.paymentId} className="border border-gray-300">
                ...
                <td
                  className={`p-1.5 font-semibold ${getStatusClass(
                    payment.status
                  )}`}
                >
                  {payment.status}
...
// Table header component for better readability
const TableHeader = () => (
  <tr className="border border-gray-300">
    <th className="p-1.5 font-semibold text-sky-900 opacity-75">No</th>
    <th className="p-1.5 font-semibold text-sky-900 opacity-75">Flat No</th>
    <th className="p-1.5 font-semibold text-sky-900 opacity-75">Amount</th>
    <th className="p-1.5 font-semibold text-sky-900 opacity-75">
      Payment Date
    </th>
    <th className="p-1.5 font-semibold text-sky-900 opacity-75">Status</th>
  </tr>
);
